...
# ---------------------------------------
if (param('savedata')) {
	$conf = new Config::Simple("$home/config/plugins/$psubfolder/husqvarna.cfg");
	...
	$conf->param('HUSQVARNA.USERNAME', unquotemeta($username));
	$conf->param('HUSQVARNA.PASSWORD', unquotemeta($password));
	$conf->param('HUSQVARNA.MINISERVER', unquotemeta($miniserver));
	$conf->param('HUSQVARNA.UDPPORT', unquotemeta($msudpport));
	$conf->param('HUSQVARNA.ENABLED', unquotemeta($enabled));
	$conf->param('HUSQVARNA.LOCALTIME', unquotemeta($localtime));
...
# ---------------------------------------
$conf = new Config::Simple("$home/config/plugins/$psubfolder/husqvarna.cfg");
$username = encode_entities($conf->param('HUSQVARNA.USERNAME'));
$password = encode_entities($conf->param('HUSQVARNA.PASSWORD'));
$miniserver = encode_entities($conf->param('HUSQVARNA.MINISERVER'));
$msudpport = encode_entities($conf->param('HUSQVARNA.UDPPORT'));
$enabled = encode_entities($conf->param('HUSQVARNA.ENABLED'));
$localtime = encode_entities($conf->param('HUSQVARNA.LOCALTIME'));
